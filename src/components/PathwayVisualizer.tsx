...
export const PathwayVisualizer = forwardRef<PathwayVisualizerRef, PathwayVisualizerProps>(({
    ...
    const handleExportSVG = async () => {
        if (!isPro) {
            alert(t('SVG (Vector) Export is a Pro feature. Please upgrade to BioCopilot Pro to unlock vector graphics, editable reports, and data saving.'));
            return;
        }
        ...
        try {
            ...
            const footerWatermark = `<text x="98%" y="98%" text-anchor="end" fill="#94a3b8" font-family="sans-serif" font-size="12">${t('Generated by BioCopilot')}</text>`;
            ...
            if (!isPro) {
                // Diagonal "BioCopilot Free" pattern
                ...
                <defs>
                    <pattern id="${patternId}" width="200" height="200" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                        <text x="100" y="100" text-anchor="middle" fill="rgba(150, 150, 150, 0.15)" font-size="24" font-family="sans-serif" font-weight="bold">${t('BioCopilot Free')}</text>
                    </pattern>
                </defs>`;
    ...
    const handleExportPPTX = async () => {
        ...
        try {
            ...
            const contactText = `${t('BioCopilot')} • biocopilot@biocopilot.com`;
            ...
            titleSlide.addText(t('BioCopilot'), { ...titleStyle, y: 2.5, fontSize: 36, color: '5DADE2' });
            ...
            titleSlide.addText(t('Generated by BioCopilot'), { x: 0.5, y: 5.0, w: 9, fontSize: 14, color: '#666666', align: 'center' });
            // Watermark for Title Slide (if Free)
            if (!isPro) {
                titleSlide.addText(t('TRIAL VERSION - BioCopilot Free'), {
                    x: 0, y: 0, w: '100%', h: '100%',
            ...
            methodSlide.addText([
                ...
                { text: 'BioCopilot v0.1', options: { fontSize: 18, color: accentColor, bold: true } },
            ...
            refSlide.addText([
                { text: `1. ${t('BioCopilot')}: ${t('An advanced, secure, and beautiful pathway visualization tool.')}`, options: { fontSize: 14, color: bodyColor, breakLine: true } },
                { text: `\n\n${t('WeChat')}:`, options: { fontSize: 14, color: accentColor, bold: true, breakLine: true } },
                { text: 'biocopilot', options: { fontSize: 14, color: '#4ecdc4' } }
