...
export const IntelligenceDashboard: React.FC<IntelligenceDashboardProps> = ({
    ...
    const layers = data.layers || {
        multi_omics: { active: false, note: t('No data') },
        temporal: { active: false, note: t('No data') },
        druggability: { active: false, hits: [] },
        topology: { active: false, note: t('No data') },
        qc: { status: 'UNKNOWN', inflation: false, variance: 0, note: t('No data') },
        lab: { active: false, recommendations: [] },
        rag_hints: { hints: [] }
    };

    const summary = data.summary || t('No analysis results available.');
        ...
        <div className="studio-dashboard-left">
            <div className="dashboard-header">
                <div className="header-main">
                    <h2><span className="icon">🧠</span> {t('Biologic Studio Intelligence')}</h2>
                ...
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    ...
                    >
                        {isGenerating ? t('⌛ Synthesizing layers...') : (data.super_narrative ? t('🔄 Regenerate Narrative') : t('✨ AI Super Narrative'))}
                    </button>
                </div>
            </div>
                        ...
                        {layers.multi_omics.active ? (
                            <>
                                <div className="synergy-stat">
                                    ...
                                    <span className="label">{t('Validation Rate')}</span>
                                </div>
                ...
                <div className={`insight-card ${layers.druggability.active ? 'active' : 'inactive'}`}>
                    ...
                    <div className="card-body">
                        {layers.druggability.active ? (
                            ...
                            <div className="empty-hint">{t('No significant druggable targets identified.')}</div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="card-body">
                        ...
                        <div className="qc-metrics">
                            <div className="metric">
                                <label>{t('P-Inflate')}</label>
                                <span>{layers.qc.inflation ? t('DETECTED') : t('NONE')}</span>
                            </div>
                            <div className="metric">
                                <label>{t('LogFC Var')}</label>
                ...
                <div className={`insight-card lab-card ${layers.lab.active ? 'active' : 'inactive'}`}>
                    ...
                    <div className="card-body">
                        {layers.lab.active ? (
                            ...
                            <div className="empty-hint">{t('Insufficient variance for recommendations.')}</div>
                        )}
                    </div>
                </div>
                ...
                {rightPanelTab === 'summary' ? (
                    <div className="studio-summary-panel">
                        {data.super_narrative ? (
                            <div className="studio-narrative-content">

                                {renderEvidenceContent(data.super_narrative)}
                            </div>
                        ) : (
                            <div className="summary-placeholder">
                                <span className="icon">✨</span>
                                <p>{t('AI Super-Narrative will appear here after synthesis.')}</p>
                                <button
                                    className="ai-synthesize-btn-inline"
                                    onClick={onGenerateSuperNarrative}
                                    disabled={isGenerating}
                                >
                                    {isGenerating ? t('Synthesizing...') : t('Synthesize Now')}
                                </button>
                            </div>
                        )}
                    </div>
